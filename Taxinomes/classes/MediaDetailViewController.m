...
- (void)viewDidAppear:(BOOL)animated {
    ...
    BOOL isLoadingAuthor =  [dm getAuthorAsychIfNeededWithId:self.media.authors.identifier withDelegate:self];
    if(!isLoadingMedia
       && !isLoadingAuthor) {
...
- (void)refreshView {
    ...
    NSString * authorAvatarUrl = media_.authors.avatarURL?media_.authors.avatarURL:@"";
    ...
    authorNameLabel_.text = [NSString stringWithFormat:@"Publié par %@ le %@\n%d vues", media_.authors.name, [df stringFromDate:self.media.date],[self.media.visits integerValue]];
    ...
    mediaPinAnnotation.subtitle = [NSString stringWithFormat:@"Par %@",media_.authors.name];
